 ...
 * attrs.xml
 * 
 * 使用时需要实现接口OnSegmentControlClickListener
 */
    ...
    public void onDraw(Canvas canvas){
        ...
        if(mTexts != null && mTexts.length > 0){
            for(int i = 0; i < mTexts.length; i++){
                ...
                //draw texts
                canvas.drawText(mTexts[i], mCacheBounds[i].left + (mSingleChildWidth - mTextBounds[i].width()) / 2, mCacheBounds[i].top + ((mSingleChildHeight + mTextBounds[i].height()) / 2)-15, mPaint);
            }
        }
